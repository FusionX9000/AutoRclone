...
def _create_remaining_accounts(iam, project):
    ...
    while sa_count != MAX_KEYS:
        _create_accounts(iam, project, MAX_KEYS - sa_count)
        sa_count = len(_list_sas(iam, project))
...
def _create_sa_keys(iam, projects, path):
    ...
    for i in projects:
        ...
        while current_key_dump is None or len(current_key_dump) != MAX_KEYS:
...
if __name__ == "__main__":
    ...
    parse.add_argument(
        "--token", default="token.pickle", help="Specify the pickle token file path."
    )
    parse.add_argument(
        "--credentials",
